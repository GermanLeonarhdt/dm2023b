...
PARAM  <- list()
PARAM$experimento  <- "HT7510EF17"

PARAM$exp_input  <- "TS7410EF17"

PARAM$lgb_crossvalidation_folds  <- 5  #En caso que se haga cross validation, se usa esta cantidad de folds

PARAM$lgb_semilla  <- 700573   #cambiar por su propia semilla
...
PARAM$bo_lgb <- makeParamSet(
         ...
         makeNumericParam("feature_fraction", lower=    0.01, upper=     1.5),
         makeIntegerParam("num_leaves",       lower=    8L,   upper=  1024L),
         makeIntegerParam("min_data_in_leaf", lower=  100L,   upper= 50000L),
        makeIntegerParam("min_gain_to_split", lower=  0.0,   upper= 20.0),
        makeIntegerParam("lambda_l1", lower=  0.0,   upper= 600.0),
        makeIntegerParam("lambda_l2", lower=  0.0,   upper= 600.0)
        )
